...
setup('Authenticate Session State Programmatically', async ({ request, page }) => {
  ...
  if (!username || !password) {
    throw new Error(
      '❌ [Auth Setup] Core Failure: APP_USER or APP_PASSWORD is missing from your .env configuration.',
    );
  }

  Logger.info(`[Auth Setup] Generating token exchange for target user: ${username}`);
  ...
  await page.context().addCookies([
    {
      ...
      domain: new URL(process.env.BASE_URL || 'https://dummyjson.com').hostname, // Map directly to the target system app domain context
      path: '/',
    },
  ]);
